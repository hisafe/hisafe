...
import safe.model.SafeFineRoot;
...
/**
  ...
  */
public class SafeExport extends StandRecordSet {
...
//		}
	...
	private void createStepTreeRecords(TreeSet dest, SafeExportVariable[] variables, Step step, int stepId, Object object) {
		...
		//if (julian > 365) julian = julian - 365;
		write.append(step.getProject().getName()+ '\t' +step.getScene().getCaption()+ '\t' +ts[0] +  '\t' +ts[1] +  '\t'+ts[2] +   '\t'+julian +  '\t');	//il - 24-04-2015
		...
		for (int i=0 ; i < variables.length ; i++) {
			...
			if (!readed) {
				...
				val=new StringBuffer("");
				SafeFineRoot safeRoot=null;
			 	safeRoot = ((SafeTree) object).getFineRoots();
			...
			if (!readed) {
				...
				if (safeCrop != null) {
					...
					if (!readed) {
						...
						val=new StringBuffer("");
						 SafeFineRoot safeRoot=null;
						 safeRoot = (SafeFineRoot) (safeCrop.getFineRoots());
	...
	private String createHeadLine(SafeExportVariable[] variables,Object o) {
		...
		write.append("SimulationName" + '\t' + "Date"+ '\t' + "Day" + '\t' + "Month" + '\t'+ "Year" + '\t'+ "JulianDay" + '\t');
		// for all variables...
		for (int i=0 ; i < variables.length ; i++) {
			readed=false;
			sizeVariableArray=0;
