...
public class SafeSticsParametersFormat extends RecordSet {
	...
	// Parameters fertilizer record is described here  fmin1
	@Import
	...
	// Parameters residus record is described here
	@Import
	...
	*/
	public void load (SafeInitialParameters settings,  SafeSticsTransit transit, SafeSticsParameters param) throws Exception {
		...
		for (Iterator i = this.iterator (); i.hasNext ();) {
			...
			if (record instanceof SafeSticsParametersFormat.ParameterFertilizerRecord) {
				...
				//TRANSIT
					...
					transit.P_codecalferti = r.getIntValue ();
				}	
				else if (r.key.equals ("codetesthumN")) {
					transit.P_codetesthumN = r.getIntValue ();
				}	
				else if (r.key.equals ("dosimxN")) {
					transit.P_dosimxN = r.getFloatValue ();
				}	
				else if (r.key.equals ("ratiolN")) {
					transit.P_ratiolN = r.getFloatValue ();
				}	
		...
		//On recopie dans STICS les valeurs de paramètres que l'on a déjà dans Hisafe
