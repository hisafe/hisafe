...
/**
 * SafeSticsSoil - Mirror object of STICS FORTRAN 
 * 
 * 
...
public class SafeSticsSoil extends Structure implements Serializable {
  ...
  public float aevap; 
  public float amm[];
  public float nit[];  			// pourquoi demarrer à 0 ?
...
//MODIF HISAFE on enleve les chaines de caractère
...
//MODIF HISAFE déplacement de Stics.f90 vers Sol.f90
  ...
  public SafeSticsSoil (SafeSoil s) {
	  ...
	  P_concseuil = (float)  s.getMinNo3Concentration ();		//kg.ha-1 mm-1
	  ...
	  //Conversion pour coller à l'unité de STICS
	...
	*/
  public void initialise (SafeSoil s, SafeInitialValues initialValues) {
		...
		for (int i=0; i<5; i++) {
			...
			if (s.getLayer(i) != null) {
				...
				if (s.getLayer(i).getBulkDensityFineSoil () > 0 ) {			
					P_Hinitf[i] 	= (float) (initialValues.layerWaterContent[i] * 100 / s.getLayer(i).getBulkDensityFineSoil ());	
				}

				P_NO3initf[i]  = (float) initialValues.layerNo3Content[i];	//kg h-1
				P_NH4initf[i] = (float) initialValues.layerNh4Content[i];	//kg h-1		
			}
		}
  	}
  
	/**
	* Soil intialisation for other simulation  
	*/
	public void reinitialise (SafeSticsReinit origin) {
		  
	
		System.arraycopy(origin.HR, 0, this.P_Hinitf, 0, 5);
		System.arraycopy(origin.AZnit, 0, this.P_NO3initf, 0, 5);
		System.arraycopy(origin.AZamm, 0,this.P_NH4initf, 0, 5);
		System.arraycopy(origin.HR, 0, this.Hinit, 0, 5);
		System.arraycopy(origin.AZnit, 0, this.NO3init , 0, 5);
		System.arraycopy( origin.AZamm, 0,this.NH4init, 0, 5);
