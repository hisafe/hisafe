...
public class SafeFineRoot implements   Serializable {
	...
	* Root constructor
	*/
	public SafeFineRoot (Object plant)  {
		createImmutable ();
	...
	/**
	* Constructor for cloning a SafeFineRoot
	*/
	public SafeFineRoot (SafeFineRoot rootModel) {
	...
	*
	*/
	public void calculatePotential (SafeInitialParameters settings, double potentialWaterDemand) {
		...
		//Tree species
  		...
  		if (this.getPlant() instanceof SafeCrop) {
			  campbellFactor = ((SafeCrop) this.getPlant()).getCropSpecies().getCampbellFactorIcraf();
			  halfCurrWaterPotential =  ((SafeCrop) this.getPlant()).getCropSpecies().getHalfCurrWaterPotentialIcraf();
  		}
	...
	*/
	...
	public double calculateWaterUptake (SafeStand stand, Object plant, SafeInitialParameters settings, double actualWaterDemand,int day) {
		...
		while (itr.hasNext()) {
			...
			SafeVoxel voxel = (SafeVoxel) itr.next();
			...
			if (waterUptake > 0) {
				...
				if (this.getPlant() instanceof SafeCrop) {
					...
					if (crop.getFineRoots().getRootTopology ().containsKey(voxel)) {
						SafeRootNode rootNode = crop.getFineRoots().getRootTopology (voxel);
					...
					if (tree.getFineRoots().getRootTopology().containsKey(voxel)) {
						SafeRootNode node = tree.getFineRoots().getRootTopology (voxel);
	...
	*/
	public void calculateNitrogenSinkStrength (SafeInitialParameters settings, double nitrogenDemand) {
	...
	*/
	...
	public double calculateNitrogenUptake (SafeStand stand, Object plant, SafeInitialParameters settings, double nitrogenDemand) {
		...
		while (itr.hasNext()) {
			...
			if (nitrogenUptake > 0) {
				...
				if (this.getPlant() instanceof SafeCrop) {
					...
					if (crop.getFineRoots().getRootTopology().containsKey(voxel)) {
						SafeRootNode node = crop.getFineRoots().getRootTopology (voxel);
...
//		if (rootedVoxelMap == null) rootedVoxelMap = new HashMap();
	...
	*/
    ...
    //We have decides that Root node is not cloned
  	...
  	public void cloneNode (HashMap after, SafeRootNode node) {
  ...
  //ROOT TOPOLOGY
	...
	public void addTreeRootTopology (SafeTree tree, SafeVoxel voxel, SafeVoxel parentVoxel, int day, double fineRootDensity, int direction) {
		...
		SafeRootNode node = new SafeRootNode (voxel, parentNode, day, fineRootDensity, tree, direction);
	...
	public void addCropRootTopology (SafeVoxel voxel, SafeVoxel parentVoxel, int day, double fineRootDensity) {
		...
		SafeRootNode node = new SafeRootNode (voxel, parentNode, day, fineRootDensity);
		rootTopology.put (voxel, node);
  		//no parent = this node is the first one
  		if (parentVoxel == null)
  			this.firstRootNode = node;
  	...
  	public void addEmptyRootTopology (SafeVoxel voxel) {
		if (rootTopology == null) rootTopology = new LinkedHashMap ();
  		rootTopology.put (voxel, new SafeRootNode (voxel, null, 0, 0, null, 4));
  	}
}
